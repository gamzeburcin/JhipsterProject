...
import com.rentacar.service.RentalQueryService;
...
/**
 * REST controller for managing {@link com.rentacar.domain.Rental}.
 */
@RestController
@RequestMapping("/api")
public class RentalResource {
    ...
    /**
     * {@code SEARCH  /_search/rentals?query=:query} : search for the rental corresponding
     * to the query.
     *
     ...
     */
    @GetMapping("/_search/rentals")
    public ResponseEntity<List<RentalDTO>> searchRentals(@RequestParam String query, Pageable pageable) {
        log.debug("REST request to search for a page of Rentals for query {}", query);
        Page<RentalDTO> page = rentalService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
}
